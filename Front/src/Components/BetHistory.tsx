...
const HISTORY: BetRecord[] = [
  ...
  { id: 7,  date: "16 may 2026 · 10:15", event: "Tottenham vs Everton",                 league: "🏴󠁧󠁢󠁥󠁮󠁧󠁿 Premier League",  pick: "Tottenham",      odds: 1.80, stake: 40000,  status: "cancelada", potential: 72000  },
  ...
  { id: 10, date: "13 may 2026 · 09:00", event: "West Ham vs Leeds United",             league: "🏴󠁧󠁢󠁥󠁮󠁧󠁿 Premier League",  pick: "West Ham",       odds: 2.10, stake: 20000,  status: "perdida",   potential: 42000  },
]

const STATUS_FILTERS = ["Todas", "Ganadas", "Perdidas", "Pendientes", "Canceladas"]
const STATUS_MAP: Record<string, BetRecord["status"] | null> = {
  "Todas": null, "Ganadas": "ganada", "Perdidas": "perdida", "Pendientes": "pendiente", "Canceladas": "cancelada",
}
...
function BetHistory() {
  ...
  const filtered = HISTORY.filter(b => {
    ...
    const searchMatch = search === "" || b.event.toLowerCase().includes(search.toLowerCase()) || b.pick.toLowerCase().includes(search.toLowerCase())
    ...
    <div className="history-container">
      ...
      <div className="stats-row">
        <div className="stat-card">
          ...
          <div className="stat-info"><span className="stat-val">{HISTORY.length}</span><span className="stat-label">Total Apuestas</span></div>
        </div>
        <div className="stat-card green">
          <span className="stat-icon">✅</span>
          <div className="stat-info"><span className="stat-val">{ganadas}</span><span className="stat-label">Ganadas</span></div>
        </div>
        <div className="stat-card red">
          <span className="stat-icon">❌</span>
          <div className="stat-info"><span className="stat-val">{perdidas}</span><span className="stat-label">Perdidas</span></div>
        </div>
        <div className="stat-card gold">
          <span className="stat-icon">💰</span>
          <div className="stat-info"><span className="stat-val">${totalWon.toLocaleString("es-CO")}</span><span className="stat-label">Total Ganado</span></div>
        </div>
        <div className={`stat-card ${parseFloat(roi) >= 0 ? "green" : "red"}`}>
          <span className="stat-icon">📈</span>
          <div className="stat-info"><span className="stat-val">{roi}%</span><span className="stat-label">ROI</span></div>
        </div>
      </div>

      {/* Controles */}
      <div className="history-controls">
        <input className="search-input" placeholder="🔍 Buscar apuesta..." value={search} onChange={e => setSearch(e.target.value)} />
        <div className="status-filters">
          {STATUS_FILTERS.map(f => (
            <button key={f} className={`status-filter-btn ${activeFilter === f ? "active" : ""}`} onClick={() => setActiveFilter(f)}>{f}</button>
          ))}
        </div>
      </div>
      ...
      <div className="history-table-wrapper">
        {filtered.length === 0 ? (
          <div className="no-results"><span>🔎</span><p>No hay apuestas que coincidan.</p></div>
        ) : (
          <table className="history-table">
            ...
            <tbody>
              {filtered.map(bet => (
                <>
                  <tr key={bet.id} className={`table-row ${expandedId === bet.id ? "expanded" : ""} ${bet.status}`} onClick={() => setExpandedId(expandedId === bet.id ? null : bet.id)}>
                    ...
                    <td className={`potential-cell ${bet.status === "ganada" ? "won" : ""}`}>${bet.potential.toLocaleString("es-CO")}</td>
                    <td><span className={`status-badge ${bet.status}`}>{STATUS_LABEL[bet.status]}</span></td>
                  </tr>
                  {expandedId === bet.id && (
                        ...
                        <div className="detail-panel">
                          <div className="detail-item">
                            ...
                            <span className={`detail-val ${bet.status === "ganada" ? "positive" : "negative"}`}>
                              {bet.status === "ganada" ? `+$${(bet.potential - bet.stake).toLocaleString("es-CO")}` : bet.status === "perdida" ? `-$${bet.stake.toLocaleString("es-CO")}` : "—"}
                            </span>
                          </div>
                          <div className="detail-item">
      ...
      <div className="history-footer">Mostrando {filtered.length} de {HISTORY.length} apuestas</div>
    </div>
  )
}
